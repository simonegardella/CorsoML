...
class Dataset ():
    ...
    def Populate (self, numeropunti):
        ...
        for i in range (numeropunti):
            ...
            y = random.random()*4
            for riga in range (self.numerooutput-1,-1,-1): # Scorro all'indietro
                for colonna in range (self.numerooutput): # Scorro in avanti
                    if 4/self.numerooutput*colonna <= x < 4/self.numerooutput*(colonna+1) and 4/self.numerooutput*riga <= y < 4/self.numerooutput*(riga+1):
                        classe = riga+ colonna
                        if classe >=self.numerooutput: # Serve perchè siamo in un'aritmetica finita di ordine
                            classe -= self.numerooutput # numerooutput. Quindi lo sottraggo per rimanere nel range!
